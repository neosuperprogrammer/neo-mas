...
public final class PH102 extends PlatformHeader {
	
	
	private static final long serialVersionUID = 539096284934851846L;

	public PH102() {
		super();
		setValue(VERSION, "PHV102");
	}
	...
	private void setWifiMac(boolean isByteBufferMode) {
		if(isByteBufferMode) {
			byte[] buf = new byte[WIFI_MAC_LENGTH];
			buffer.get(buf);
			setValue(WIFI_MAC, new String(buf));
		} else {
			try {
				ByteBuffer buf = ByteBuffer.allocate(WIFI_MAC_LENGTH);
				fc.read(buf);
	...
	private void setUuid(boolean isByteBufferMode) {
		if(isByteBufferMode) {
			...
			try {
				ByteBuffer buf = ByteBuffer.allocate(UUID_LENGTH);
				fc.read(buf);
